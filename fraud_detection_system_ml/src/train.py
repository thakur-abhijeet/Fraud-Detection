...
class FraudDetectionModel:
    ...
    def _create_visualizations(self, X_val, y_val):
        ...
        xgb_fpr, xgb_tpr, _ = roc_curve(
            y_val, self.xgb_model.predict_proba(X_val)[:, 1]
        )
        lgb_fpr, lgb_tpr, _ = roc_curve(
            y_val, self.lgb_model.predict_proba(X_val)[:, 1]
        )
        ...
        plt.plot(
            xgb_fpr,
            xgb_tpr,
            label=f"XGBoost (AUC = {roc_auc_score(y_val, self.xgb_model.predict_proba(X_val)[:, 1]):.3f})",
        )
        plt.plot(
            lgb_fpr,
            lgb_tpr,
            label=f"LightGBM (AUC = {roc_auc_score(y_val, self.lgb_model.predict_proba(X_val)[:, 1]):.3f})",
        )
        plt.plot([0, 1], [0, 1], "k--")
    ...
    def save_models(self, path):
        ...
        self.preprocessor.save(os.path.join(path, "preprocessor.joblib"))
    ...
    def load_models(self, path):
        ...
        self.preprocessor.load(os.path.join(path, "preprocessor.joblib"))


def main():
    parser = argparse.ArgumentParser(description="Train fraud detection models")
    parser.add_argument(
        "--input", required=True, help="Input preprocessed CSV file path"
    )
    parser.add_argument(
        "--output", required=True, help="Output directory for saved models"
    )
